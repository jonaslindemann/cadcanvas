...
const EpochInformation   = 'Lambda-Epsilon';
const RecentUpdate       = '01-08-2005';
const LCID               = '$02-08-2005:AEDEAB4632950C2FAE$';
...
type eTriangletype = (Equilateral,Isosceles,Right,Scalene,Obtuse,TUnknown);
...
function CartesianAngle(x,y:TFloat):TFloat;
...
function  CreateEquilateralTriangle(x1,y1,x2,y2:TFloat):TTriangle2D;                                                     overload;
...
procedure GenerateRandomPoints(const Bx1,By1,Bx2,By2:TFloat; var Point:array of TPoint2D);                               overload;
procedure GenerateRandomPoints(const Rect:TRectangle;        var Point:array of TPoint2D);                               overload;
procedure GenerateRandomPoints(const Triangle:TTriangle2D;   var Point:array of TPoint2D);                               overload;
procedure GenerateRandomPoints(const Circle:TCircle;         var Point:array of TPoint2D);                               overload;
procedure GenerateRandomPoints(const Quadix:TQuadix2D;       var Point:array of TPoint2D);                               overload;
...
begin
 ...
 (*
  ...
  Note:
  The method below is very stable and logical, however at the same
  time it is "VERY" inefficient, it requires 3 SQRTs which is not
  acceptable...
...
begin
 Result := (Orientation(PointA,PointB,PointC,PointD) = 0);
end;
(* End of Coplanar *)
...
begin
 (* Result := (Orientation(Segment[1].x,Segment[1].y,Segment[2].x,Segment[2].y,x,y) = RightHandSide); *)
...
begin
 (* Result := (Orientation(Seg,Point)  = RightHandSide); *)
...
begin
 (* Result := (Orientation(Line[1].x,Line[1].y,Line[2].x,Line[2].y,x,y) = RightHandSide); *)
...
begin
 Result := (((px - x1) * (y2 - y1)) > ((x2 - x1) * (py - y1)));
end;
...
var
  ...
  PO : TFloat;
begin
 P1 := PointInRectangle(Segment[1],Rectangle);
 P2 := PointInRectangle(Segment[2],Rectangle);
 ...
 PO := Orientation(Segment[1].x,Segment[1].y,Segment[2].x,Segment[2].y,Rectangle[1].x,Rectangle[1].y);

 CO := Orientation(Segment[1].x,Segment[1].y,Segment[2].x,Segment[2].y,Rectangle[2].x,Rectangle[1].y);
 if CO <> PO then Exit;

 PO := CO;
 CO := Orientation(Segment[1].x,Segment[1].y,Segment[2].x,Segment[2].y,Rectangle[2].x,Rectangle[2].y);
 if CO <> PO then Exit;

 PO := CO;
 CO := Orientation(Segment[1].x,Segment[1].y,Segment[2].x,Segment[2].y,Rectangle[1].x,Rectangle[2].y);
 if CO <> PO then Exit;
...
begin
 ...
 P1 := PointInTriangle(Segment[1],Triangle);
 P2 := PointInTriangle(Segment[2],Triangle);
 ...
 CO := Orientation(Segment[1].x,Segment[1].y,Segment[2].x,Segment[2].y,Triangle[1].x,Triangle[1].y);
 PO := CO;

 CO := Orientation(Segment[1].x,Segment[1].y,Segment[2].x,Segment[2].y,Triangle[2].x,Triangle[2].y);
 if CO <> PO then Exit;
 PO := CO;

 CO := Orientation(Segment[1].x,Segment[1].y,Segment[2].x,Segment[2].y,Triangle[3].x,Triangle[3].y);
 if CO <> PO then Exit;
...
begin
 ...
 P1 := PointInQuadix(Segment[1],Quadix);
 P2 := PointInQuadix(Segment[2],Quadix);
 ...
 CO := Orientation(Segment[1].x,Segment[1].y,Segment[2].x,Segment[2].y,Quadix[1].x,Quadix[1].y);
 PO := CO;

 CO := Orientation(Segment[1].x,Segment[1].y,Segment[2].x,Segment[2].y,Quadix[2].x,Quadix[2].y);
 if CO <> PO then Exit;

 PO := CO;
 CO := Orientation(Segment[1].x,Segment[1].y,Segment[2].x,Segment[2].y,Quadix[3].x,Quadix[3].y);
 if CO <> PO then Exit;

 PO := CO;
 CO := Orientation(Segment[1].x,Segment[1].y,Segment[2].x,Segment[2].y,Quadix[4].x,Quadix[4].y);
 if CO <> PO then Exit;
 ...
 *)
 //Ln     := Translate(-Circle.x,-Circle.y,Line);
 //Result := (((Circle.Radius * Circle.Radius) * LayDistance(Line) - Sqr(Line[1].x * Line[2].y - Line[2].x * Line[1].y)) >= 0.0);
...
begin
 ICnt := 0;
 ClosestPointOnSegmentFromPoint(Segment[1].x,Segment[1].y,Segment[2].x,Segment[2].y,Circle.x,Circle.y,Px,Py);
 PCDist := Distance(Px,Py,Circle.x,Circle.y);

 if PCDist > Circle.Radius then
   Exit;

 RadSqr := Circle.Radius * Circle.Radius;

 LeyDist1 := LayDistance(Circle.x,Circle.y,Segment[1].x,Segment[1].y);
 LeyDist2 := LayDistance(Circle.x,Circle.y,Segment[1].x,Segment[2].y);


 if (LeyDist1 >= RadSqr) And (LeyDist2 >= RadSqr) then
 begin
  ICnt := ICnt or INTERSECTION_1 or INTERSECTION_2;

  Dist := Distance(Px,Py,Segment[1].x,Segment[1].y);
  ...
  begin
   Ratio := sqrt(RadSqr - PCDist * PCDist) / Dist;
   I1.x  := Px + (Segment[1].x - Px) * Ratio;
   I1.y  := Py + (Segment[1].y - Py) * Ratio;
  end;

  Dist := Distance(Px,Py,Segment[2].x,Segment[2].y);
  if IsEqual(Dist,0.0) then
  begin
   I2.x := Segment[2].x;
   I2.y := Segment[2].y;
  ...
  begin
   Ratio := sqrt(RadSqr - PCDist * PCDist) / Dist;
   I2.x  := Px + (Segment[2].x - Px) * Ratio;
   I2.y  := Py + (Segment[2].y - Py) * Ratio;
  end;
 end
 else if LeyDist1 < RadSqr then
 begin
   if LeyDist2 < RadSqr then
   begin
    ICnt := ICnt or INTERSECTION_1 or INTERSECTION_2;
    I1.x := Segment[1].x;
    I1.y := Segment[1].y;
   ...
   begin
    ICnt := ICnt or INTERSECTION_1;
    I1.x := Segment[1].x;
    I1.y := Segment[1].y;

    Dx := (Segment[2].x - Segment[1].x);
    Dy := (Segment[2].y - Segment[1].y);

    I2.x := Segment[1].x + Dx * (Circle.y - Segment[1].y) / Dy;
    I2.y := Segment[1].y + Dy * (Circle.x - Segment[1].x) / Dx;

   end;
 end
 else if LeyDist2 < RadSqr then
 begin
  ICnt := ICnt or INTERSECTION_2;
  I1.x := Segment[2].x;
  I1.y := Segment[2].y;

  Dx := (Segment[1].x - Segment[2].x);
  Dy := (Segment[1].y - Segment[2].y);

  I2.x := Segment[2].x + Dx * (Circle.y - Segment[2].y) / Dy;
  I2.y := Segment[2].y + Dy * (Circle.x - Segment[2].x) / Dx;
 end;
end;
(* End of IntersectionPoint *)
...
function CartesianAngle(x,y:TFloat):TFloat;
begin
 if      (x >  0) and (y >  0) then Result := (ArcTan( y/x) * _180DivPI)
 else if (x <  0) and (y >  0) then Result := (ArcTan(-x/y) * _180DivPI) +  90.0
 else if (x <  0) and (y <  0) then Result := (ArcTan(-y/x) * _180DivPI) + 180.0
 else if (x >  0) and (y <  0) then Result := (ArcTan(-x/y) * _180DivPI) + 270.0
 else if (x  = 0) and (y >= 0) then Result :=  90.0
 else if (x <= 0) and (y =  0) then Result := 180.0
 else if (x  = 0) and (y <  0) then Result := 270.0
 else
  Result := 0.0;
end;
...
begin
 Result := Perpendicular(Segment1[1],Segment1[2],Segment2[1],Segment2[2]);
end;
(* End of Perpendicular *)
...
begin
 Result := Perpendicular(Line1[1],Line1[2],Line2[1],Line2[2]);
end;
(* End of Perpendicular *)
...
begin
 Result := sqrt(LayDistance(Point,Triangle));
end;
(* End of Distance *)
...
begin
 Result := TriangleType(Point1,Point2,Point3);
end;
(* End of Triangletype *)
...
function CreateEquilateralTriangle(x1,y1,x2,y2:TFloat):TTriangle2D;
begin
 Result[1].x := x1;
 Result[1].y := y1;
 Result[2].x := x2;
...
begin
 if Orientation(Triangle[1],Triangle[2],Point) <> Orientation(Triangle[1],Triangle[2],Triangle[3]) then
 begin
  ClosestPointOnSegmentFromPoint(Triangle[1].x,Triangle[1].y,Triangle[2].x,Triangle[2].y,Point.x,Point.y,Result.x,Result.y);
  Exit;
 end;

 if Orientation(Triangle[2],Triangle[3],Point) <> Orientation(Triangle[2],Triangle[3],Triangle[1]) then
 begin
  ClosestPointOnSegmentFromPoint(Triangle[2].x,Triangle[2].y,Triangle[3].x,Triangle[3].y,Point.x,Point.y,Result.x,Result.y);
  Exit;
 end;

 if Orientation(Triangle[3],Triangle[1],Point) <> Orientation(Triangle[3],Triangle[1],Triangle[2]) then
 begin
  ClosestPointOnSegmentFromPoint(Triangle[3].x,Triangle[3].y,Triangle[1].x,Triangle[1].y,Point.x,Point.y,Result.x,Result.y);
  Exit;
 end;
...
var
  ...
  Or41 : Integer;
begin
 Or12 := Orientation(Quadix[1],Quadix[2],Point);
 Or41 := Orientation(Quadix[4],Quadix[1],Point);
 Or23 := Orientation(Quadix[2],Quadix[3],Point);

 if (Or41 = Or23) and (Or12 <> Or41) then
 begin
  ClosestPointOnSegmentFromPoint(Quadix[1].x,Quadix[1].y,Quadix[2].x,Quadix[2].y,Point.x,Point.y,Result.x,Result.y);
  Exit;
 end;

 Or34 := Orientation(Quadix[3],Quadix[4],Point);

 if (Or12 = Or34) and (Or23 <> Or12) then
 begin
  ClosestPointOnSegmentFromPoint(Quadix[2].x,Quadix[2].y,Quadix[3].x,Quadix[3].y,Point.x,Point.y,Result.x,Result.y);
  Exit;
 end;

 if (Or23 = Or41) and (Or34 <> Or23) then
 begin
  ClosestPointOnSegmentFromPoint(Quadix[3].x,Quadix[3].y,Quadix[4].x,Quadix[4].y,Point.x,Point.y,Result.x,Result.y);
  Exit;
 end;

 if (Or34 = Or12) and (Or41 <> Or34) then
 begin
  ClosestPointOnSegmentFromPoint(Quadix[4].x,Quadix[4].y,Quadix[1].x,Quadix[1].y,Point.x,Point.y,Result.x,Result.y);
  Exit;
 end;
...
var
  ...
  Dz      : TFloat;
begin
  if IsDegenerate(x1,y1,z1,x2,y2,z2) then
  ...
  *)
   ...
   (* Ratio := (((x1 - Px) * Dx) + ((y1 - Py) * Dy) + ((z1 - Pz) * Dz)) / (Dx * Dx + Dy * Dy + Dz * Dz); *)
   Ratio := (((Px - x1) * Dx) + ((Py - y1) * Dy) + ((Pz - z1) * Dz)) / (Dx * Dx + Dy * Dy + Dz * Dz);
   ...
   //Result := Distance(Px,Py,Pz,x1 + Ratio * (x2-x1),y1 + Ratio * (y2-y1),z1 + Ratio * (z2-z1));
     Result := Distance(Px,Py,Pz,x1 + Ratio * Dx,y1 + Ratio * Dy,z1 + Ratio * Dz);
  end;
end;
(* End of Minimum Distance From Point to Segment *)
...
begin
 Result := MinimumDistanceFromPointToLine(Line[1].x,Line[1].y,Line[2].x,Line[2].y,Point.x,Point.y);
end;
(* End of Minimum Distance From Point to Line *)
...
begin
 ...
 begin
   ...
   (*
   if (LessThanOrEqual(Polygon[i].y,Py) and (Py < Polygon[j].y)) or     // an upward crossing
      (LessThanOrEqual(Polygon[j].y,Py) and (Py < Polygon[i].y)) then   // a downward crossing
   *)
...
begin
 (*
   ...
   3.) Unclosed area
 *)
 Result := not SimplePolygon(Polygon);
end;
...
function SelfIntersectingPolygon(const Polygon:TPolygon2D):Boolean;
begin
 Result := Intersect(Polygon,Polygon);
end;
...
begin
 Result := (Rectangle[2].x - Rectangle[1].x) * (Rectangle[2].y - Rectangle[1].y);
end;
(* End of Area *)
...
function GenerateRandomValue(Range : TFloat; Resolution : TFloat = 10000.0) : TFloat;
begin
...
procedure GenerateRandomPoints(const Bx1,By1,Bx2,By2:TFloat; var Point: array of TPoint2D);
var
  i  : Integer;
  Dx : TFloat;
  Dy : TFloat;
begin
 ...
 begin
  Point[i].x := Bx1 + GenerateRandomValue(Dx);
  Point[i].y := By1 + GenerateRandomValue(Dy);
 end;
end;
(* End Generate Random Points *)


procedure GenerateRandomPoints(const Rect:TRectangle; var Point: array of TPoint2D);
begin
 GenerateRandomPoints(Rect[1].x,Rect[1].y,Rect[2].x,Rect[2].y,Point);
end;
(* End Generate Random Points *)


procedure GenerateRandomPoints(const Triangle:TTriangle2D; var Point: array of TPoint2D);
const Resolution    = 1000000;
const ResolutionFlt = Resolution * 1.0;
var
  i     : Integer;
  u     : TFloat;
  v     : TFloat;
...
begin
 ...
 for i := 0 to Length(Point) - 1 do
 begin
  RandU := Random( Resolution         );
  RandV := Random( Resolution - RandU );
  RandW := Resolution - RandU - RandV;

  u := (1.0 * RandU) / ResolutionFlt;
  v := (1.0 * RandV) / ResolutionFlt;
  w := (1.0 * RandW) / ResolutionFlt;

  Point[i].x := u * Triangle[1].x + v * Triangle[2].x + w * Triangle[3].x;
  Point[i].y := u * Triangle[1].y + v * Triangle[2].y + w * Triangle[3].y;
 end;
end;
(* End Generate Random Points *)


procedure GenerateRandomPoints(const Circle:TCircle; var Point: array of TPoint2D);
var
  i         : Integer;
  RandAngle : TFloat;
  CPoint      : TPoint2D;
begin
 ...
 for i := 0 to Length(Point) - 1 do
 begin
  RandAngle  := GenerateRandomValue(360);
  Point[i].x := Circle.x + GenerateRandomValue(Circle.Radius);
  Point[i].y := Circle.y;
  Point[i]   := Rotate(RandAngle,Point[i],CPoint);
 end;
end;
(* End Generate Random Points *)


procedure GenerateRandomPoints(const Quadix:TQuadix2D; var Point: array of TPoint2D);
var
  i         : Integer;
  P1        : array of TPoint2D;
  P2        : array of TPoint2D;
  Area1Diff : TFloat;
  Area2Diff : TFloat;
begin

 Area1Diff := Abs(Area(Quadix[1],Quadix[2],Quadix[4]) - Area(Quadix[2],Quadix[3],Quadix[4]));
 Area2Diff := Abs(Area(Quadix[1],Quadix[2],Quadix[3]) - Area(Quadix[3],Quadix[4],Quadix[1]));

 if Area1Diff <= Area2Diff then
 begin
  SetLength(P1,Round(Length(Point) * (Area(Quadix[1],Quadix[2],Quadix[4]) / Area(Quadix))));
  SetLength(P2,Length(Point) - Length(P1));
  GenerateRandomPoints(EquateTriangle(Quadix[1],Quadix[2],Quadix[4]),P1);
  GenerateRandomPoints(EquateTriangle(Quadix[2],Quadix[3],Quadix[4]),P2);
 end
 else
 begin
  SetLength(P1,Round(Length(Point) * (Area(Quadix[1],Quadix[2],Quadix[3]) / Area(Quadix))));
  SetLength(P2,Length(Point) - Length(P1));
  GenerateRandomPoints(EquateTriangle(Quadix[1],Quadix[2],Quadix[3]),P1);
  GenerateRandomPoints(EquateTriangle(Quadix[3],Quadix[4],Quadix[1]),P2);
 end;
...
begin
 ...
 repeat
  Quadix[1].x := Bx1 + Random(Dx) + (1.0 * Random(10000) / 10000.0);
  Quadix[1].y := By1 + Random(Dy) + (1.0 * Random(10000) / 10000.0);

  Quadix[2].x := Bx1 + Random(Dx) + (1.0 * Random(10000) / 10000.0);
  Quadix[2].y := By1 + Random(Dy) + (1.0 * Random(10000) / 10000.0);

  Quadix[3].x := Bx1 + Random(Dx) + (1.0 * Random(10000) / 10000.0);
  Quadix[3].y := By1 + Random(Dy) + (1.0 * Random(10000) / 10000.0);

  Quadix[4].x := Bx1 + Random(Dx) + (1.0 * Random(10000) / 10000.0);
  Quadix[4].y := By1 + Random(Dy) + (1.0 * Random(10000) / 10000.0);
 until (not IsDegenerate(Quadix)) and ConvexQuadix(Quadix);
end;
(* End Generate Random Quadix *)
...
begin
 Dx := Abs(Bx2 - Bx1) - 1.0;
 Dy := Abs(By2 - By1) - 1.0;
 Circle.Radius := Random(Round(Min(Dx,Dy) * 0.5)) + (1.0 * Random(10000) / 10000.0);
 Circle.x      := Bx1 + Circle.Radius + Random(Round(Dx - (2 * Circle.Radius))) + (1.0 * Random(10000) / 10000.0);
 Circle.y      := By1 + Circle.Radius + Random(Round(Dy - (2 * Circle.Radius))) + (1.0 * Random(10000) / 10000.0);
end;
(* End Generate Random Circle *)
